...
object CalendarReader {
    ...
    private fun instancesUri(fromMillis: Long, toMillis: Long): Uri {
        return android.provider.CalendarContract.Instances.CONTENT_URI.buildUpon()
    ...
    fun queryEvents(
        ...
        fromMillis: Long,
        toMillis: Long,
        settings: com.shiftalarm.app.data.AppSettings
    ): List<CalEvent> {
        val projection = arrayOf(
    ...
    fun countAllEventsPerCalendar(context: Context): Map<Long, Int> {
        val result = mutableMapOf<Long, Int>()
        runCatching {
            context.contentResolver.query(
                android.provider.CalendarContract.Events.CONTENT_URI,
                arrayOf(android.provider.CalendarContract.Events.CALENDAR_ID),
                null, null, null
            )?.use { c ->
                while (c.moveToNext()) {
    ...
    fun upcomingEventsAnyDate(context: Context, limit: Int): List<EventPreview> {
        val result = mutableListOf<EventPreview>()
        ...
        runCatching {
            context.contentResolver.query(
                ...
                while (c.moveToNext() && result.size < limit) {
                    result += EventPreview(c.getString(2) ?: "", c.getLong(1), c.getLong(0))
                }
            }
        }
        return result
    ...
    private fun lookupLocation(context: Context, eventId: Long): String {
        return runCatching {
            context.contentResolver.query(
                android.provider.CalendarContract.Events.CONTENT_URI,
                arrayOf(android.provider.CalendarContract.Events.EVENT_LOCATION),
